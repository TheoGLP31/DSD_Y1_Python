    ...
    while flag:
        ...
        try:
           datetime.strptime(user_date, "%d/%m/%Y").date()
            ...
            splitCurrentDate = date.split("-")
            splitInputDate = user_date.split("/", -1)

            print(splitCurrentDate, splitInputDate)
            print(splitInputDate[2] > splitCurrentDate[0])

            for i in range(len(splitCurrentDate)):
                splitCurrentDate[i] = int(splitCurrentDate[i])
                
            for i in range(len(splitInputDate)):
                splitInputDate[i] = int(splitInputDate[i])
            ...
            if (int(splitCurrentDate[0]) > int(splitInputDate[2])) or (int(splitCurrentDate[1]) > int(splitInputDate[1]) and int(splitCurrentDate[2] > splitInputDate[0])):
                flag = True
